...
export class SpeciesService {
  ...
  async findAll() {
    ...
    species.forEach((species) => {
      species.homeworld = species.homeworld
        ? species.homeworld.map((planet) => planet.url)
        : [];
      species.people = species.people
        ? species.people.map((person) => person.url)
        : [];
      species.films = species.films
        ? species.films.map((film) => film.url)
        : [];
    });
